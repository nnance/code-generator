...
Confirmed setup: rapid-mlx at `http://127.0.0.1:8001/v1`, model `qwen3.8-27b-4bit`, advertised context capacity 262144 tokens, and a one-hour active-time default. Live integration results are recorded in IMPLEMENTATION.md. The package uses `@nicknance/code-generator` locally; npm publishing and namespace availability remain unverified. Existing personal skill collections can be tried through repository-local `.agents/skills`; the automated suite validates representative skill fixtures.
