...
export async function assess(store: Store, budget: Budget, fetcher: typeof fetch) {
  ...
  const agent = new ToolLoopAgent({ model: model(store.state.config, fetcher), maxRetries: 0, providerOptions: store.state.config.providerOptions, maxOutputTokens: 1024,
